...
class Channel:
    UPLINK_FREQUENCY = 15 # Carrier Frequency [GHz]
    DOWNLINK_FREQUENCY = 10  # Carrier Frequency [GHz]
    ...
    def __init__(self, device_lat, device_lon, satellite_lat, satellite_lon, distance, elevation_angle, eirp_gt, eirp_satellite, gain_to_noise_gt, gain_to_noise_satellite, bandwidth):
    ...
    def get_path_loss(self):
        ...
        fspl = self.FSPL_CONSTANT + 20 * math.log10(self.distance) + 20 * math.log10(self.DOWNLINK_FREQUENCY)

        # Atmospheric attenuation
        atm_loss = itur.atmospheric_attenuation_slant_path(
            self.device_lat, self.device_lon, self.DOWNLINK_FREQUENCY, self.elevation_angle,
        ...
        # Add shadowing to the path loss
        path_loss = fspl + atm_loss

        return path_loss

    def get_snr_dl(self):
        snr = self.eirp_satellite + self.gain_to_noise_gt - self.get_path_loss() - self.BOLTZ_CONST - 10 * math.log10(self.bandwidth)
        snr = round(snr, 4)
  
        return snr

    def get_snr_ul(self):
        snr = self.eirp_gt + self.gain_to_noise_satellite - self.get_path_loss() - self.BOLTZ_CONST - 10 * math.log10(self.bandwidth)
        snr = round(snr, 4)
    ...
    def compute_dl_rate(self):
        ...
        dl_rate = self.bandwidth * math.log2(1 + 10 ** (dl_SNR / 10))

        return dl_rate

    def compute_ul_rate(self):
        ul_SNR = self.get_snr_ul()  # Calculate your uplink SNR
        ul_rate = self.bandwidth * math.log2(1 + 10 ** (ul_SNR / 10))
